GO
DROP DATABASE IF EXISTS [Significant_Well_Being_DW];

GO
CREATE DATABASE [Significant_Well_Being_DW];

GO
USE [Significant_Well_Being_DW];
...
DROP TABLE IF EXISTS dbo.DimNurse
GO
CREATE TABLE dbo.DimNurse
(
	NurseKey INT IDENTITY,
	NurseID INT,
...
DROP TABLE Nurse_Stage
GO
CREATE TABLE Nurse_Stage
(
	NurseID INT,
	NurseUserName nvarchar(20) NOT NULL,
	NurseGroupName NVARCHAR(50) NOT NULL,
...
CREATE PROCEDURE dbo.Nurse_Extract
AS
BEGIN
	SET NOCOUNT ON;
	SET XACT_ABORT ON;
	...
	INSERT INTO dbo.Nurse_Stage
	SELECT	   N.NurseID,
			   N.NurseUserName,
			   N.NurseGroupName
		FROM Significant_Well_Being.dbo.Nurse N 
...
EXECUTE dbo.Nurse_Extract
GO
SELECT * FROM Nurse_Stage
GO
SELECT * FROM DimNurse
GO
-- ======================= SSIS ANALYSIS FOR DimNurse SCD TYPE 2 =========================
--SELECT * FROM Nurse_Stage WHERE NurseUserName='User1'
--SELECT * FROM DimNurse WHERE NurseUserName='User1'
--UPDATE Nurse_Stage SET NurseGroupName='Intake Care Coordinators' WHERE NurseUserName='User1' 
-- ================= X ======================== X ============================ X =========
...
BEGIN
	...
	WITH PatientDetails AS (SELECT P.PatientID,
							...
							FROM Significant_Well_Being.dbo.Patient P
							LEFT JOIN Significant_Well_Being.dbo.PatientType PAT ON PAT.PatientTypeID = P.PatientTypeID
							LEFT JOIN Significant_Well_Being.dbo.PriorityType PRT ON PRT.PriorityTypeID = P.PriorityTypeID
							LEFT JOIN Significant_Well_Being.dbo.Patient_PatientStatus PPS ON PPS.PatientID = P.PatientID
							LEFT JOIN Significant_Well_Being.dbo.PatientStatus PAS ON PAS.StatusID = PPS.StatusID)
	...
	INNER JOIN Significant_Well_Being.dbo.[Well_Being_RawData_200] RIT ON RIT.ClientID = P.PatientID
	INNER JOIN Significant_Well_Being.dbo.Department D ON D.DepartmentID = P.PatientReferralCategoryID
	INNER JOIN Significant_Well_Being.dbo.Locations L ON L.LocationID = P.LocationID
...
BEGIN
	...
	WITH DepartmentDetails AS (SELECT D.DepartmentName,
									  L.LocationName
							FROM Significant_Well_Being.dbo.Department D
							LEFT JOIN Significant_Well_Being.dbo.DepartmentLocation DL ON DL.DepartmentID = D.DepartmentID
							LEFT JOIN Significant_Well_Being.dbo.Locations L ON L.LocationID = DL.LocationID
...
*/
GO
CREATE OR ALTER PROCEDURE dbo.Patients_Transform
AS
BEGIN
	SET NOCOUNT ON;
	SET XACT_ABORT ON;
	...
	BEGIN TRANSACTION
		...
		SELECT NEXT VALUE FOR dbo.Seq_Patient_Preload AS PatientKey,
			   ...
			   NULL
		FROM dbo.Patient_Stage stg
		WHERE NOT EXISTS (SELECT 1 FROM Significant_Well_Being_DW.dbo.DimPatient DP
...
BEGIN
	...
	BEGIN TRANSACTION
		...
		SELECT NEXT VALUE FOR dbo.Seq_Nurse_Preload AS NurseKey,
			   ...
			   NULL
		FROM dbo.Patient_Stage stg
		LEFT JOIN Significant_Well_Being.dbo.Nurse N ON N.NurseUserName = stg.NurseID
...
END;
GO
EXECUTE Nurses_Transform
GO
SELECT * FROM Nurse_Preload
GO
/*
...
GO
CREATE OR ALTER PROCEDURE dbo.FactPatients_Extract
AS
BEGIN
	SET NOCOUNT ON;
	SET XACT_ABORT ON;
	...
	TRUNCATE TABLE Significant_Well_Being_DW.dbo.FactPatient_Stage;
	...
	WITH PatientDetails AS (SELECT P.PatientID,
							...
							FROM Significant_Well_Being.dbo.Patient P
							LEFT JOIN Significant_Well_Being.dbo.Well_Being_RawData_200 RIT ON RIT.ClientID = P.PatientID
							LEFT JOIN Significant_Well_Being.dbo.Department D ON D.DepartmentName = RIT.Referral_Category
							LEFT JOIN Significant_Well_Being.dbo.Patient_PatientStatus PPS ON PPS.PatientID = P.PatientID)

	INSERT INTO Significant_Well_Being_DW.dbo.FactPatient_Stage
	SELECT PatientID,
		   DepartmentName,
		   CreatedAtDateTime,
		   MarkedCompletedOnDate,
...
END;
GO
EXECUTE FactPatients_Extract
GO
SELECT * FROM FactPatient_Stage
GO
/*
...
GO
CREATE OR ALTER PROCEDURE dbo.FactPatients_Transform
AS
BEGIN
	SET NOCOUNT ON;
	SET XACT_ABORT ON;
	...
	BEGIN TRY
	BEGIN TRANSACTION
		TRUNCATE TABLE Significant_Well_Being_DW.dbo.FactPatient_Preload;
		...
		(
			SELECT pat_preload.PatientUsername
				   ...
				   --,COUNT(*) AS COUNT
			FROM Significant_Well_Being.dbo.Patient pat
			JOIN Significant_Well_Being.dbo.Department dep ON dep.DepartmentID = pat.PatientReferralCategoryID
...
EXECUTE FactPatients_Transform
GO
